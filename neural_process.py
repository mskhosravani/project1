import torch
from model import Encoder, MuSigmaEncoder, Decoder, REncoder, ZEncoder, Decoder_
from torch import nn
from torch.distributions import Normal
from utils import img_mask_to_np_input

import torch
from model import Encoder, MuSigmaEncoder, Decoder
from torch import nn
from torch.distributions import Normal
from utils import img_mask_to_np_input
...
class NP(nn.Module):
    ...
    def __init__(self, img_size, r_dim, z_dim, h_dim):
        ...
        self.h_dim = h_dim
        x_dim = 2
        self.repr_encoder = REncoder(x_dim + self.num_channels, h_dim, r_dim)  # (x,y)->r
        self.z_encoder = ZEncoder(r_dim, z_dim)  # r-> mu, std
        self.decoder = Decoder_(x_dim + z_dim, h_dim, h_dim, self.num_channels)  # (x*, z) -> y*
    ...
    def data_to_z_params(self, x, y):
        ...
        r_agg = rs.mean(dim=1)  # Average over samples
    ...
    def forward(self, x_context, y_context, x_target, y_target):
        ...
        z_mean_all, z_std_all = self.data_to_z_params(x_target, y_target)
        z_mean_context, z_std_context = self.data_to_z_params(x_context, y_context)
        #Sample a batch of zs using reparam trick.
        zs = self.sample_z(z_mean_all, z_std_all, 1)

        mu, std = self.decoder(x_target, zs)
        return mu, std, z_mean_all, z_std_all, z_mean_context, z_std_context